...
After running the init command, copy the values from `.env.submit` to `.env`:
...
- **`.env`** - Contains credentials for `chrome-webstore-upload-cli`
- **`.env.submit`** - Generated by WXT with `CHROME_*` prefixed variables
- Both files are in `.gitignore` - never commit them!

## Notes

- ✅ Extension will be published automatically with `--auto-publish` flag
- ⚠️ Remove `--auto-publish` from package.json if you want manual approval
- 📝 The zip file location: `.output/chrome-mv3.zip`
